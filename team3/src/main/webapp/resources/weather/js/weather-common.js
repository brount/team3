jQuery(document).ready(function(){
	jQuery(this).on('mouseup', function(e) {
		var container = jQuery('#lnb');
		if(!container.is(e.target) && container.has(e.target).length == 0) {
			container.find('ul').each(function(idx,ele){
				if(ele.id.substring(0,12) == "top-sub-menu"){
					jQuery(ele).hide();
				}
			});
			container.find('a[id^="top-menu-head"]').each(function(idx){
...
function setCookieBase64( name, value, eds ){
	...
	document.cookie = name + "=" +  Base64.encode(value)  + "; path=/; expires=" + td.toGMTString() +";";
}
...
/* Tab over,out */
function initImgEffect(elementId,selectedTab) {
	
	var menuImgs = document.getElementById(elementId).getElementsByTagName("img");
	if(menuImgs && menuImgs.length > 0) {
		var imgLen = menuImgs.length;

		for (var i=0; i < imgLen; i++) {
			cacheToggleImage(menuImgs.item(i).src);
			menuImgs.item(i).onmouseover = menuOver;
			menuImgs.item(i).onmouseout = menuOut;
			if (i == selectedTab) {
				menuImgs.item(i).onmouseover();
				menuImgs.item(i).onmouseover = null;
				menuImgs.item(i).onmouseout = null;
			}
		}
	}
	var menus = document.getElementById(elementId).getElementsByTagName("a");
	if(menus && menus.length > 0) {
		var menuLen = menus.length;

		for (var i=0; i < menuLen; i++) {
			if (i == selectedTab) {
				menus.item(i).className = menus.item(i).className + " active";
			}
		}
	}
...
function initTopMenu(el,depth1) {
	var subImgs = el.getElementsByTagName("img");
	if(subImgs && subImgs.length > 0) {
		...
		jQuery(el).find("a[id^='top-menu-head']").each(function(idx, ele) {
			if(ele.id == "top-menu-head" + depth1) {
...
function topMenuOver(img) {
	if(img && img.src) img.src = img.src.replace(".gif", "_on.gif");
}
function topMenuOut(img) {
	if(img && img.src) img.src = img.src.replace("_on.gif", ".gif");
}
function selectTopmenuByMenuId() {
	...
	jQuery(nav).on('mouseleave', function() {
		var $me = jQuery(this);
		$me.hide();
		$me.parent().find("a").first().removeClass("selected");
	});
	menuEl = nav.getElementsByTagName("li");
	for(var i = 0; i < menuEl.length; i++) {
		var mEl = menuEl.item(i);
		var aEl = mEl.getElementsByTagName("a");
		mEl.onmouseover = function(e) {
			jQuery(this).addClass("active");
		};
		mEl.onmouseout = function(e) {
			jQuery(this).removeClass("active");
		};
		
		if(aEl != null && aEl.length>0) {
			var found = false;
			jQuery(mEl).find("ul").each(function(idx, ele){
				if(idx == 0) {
					if(!jQuery(aEl[0]).hasClass("addon")) {
						jQuery(aEl[0]).addClass("addon");
					}
					found = true;
				}
			});
			if(!found) {
				if(jQuery(aEl[0]).hasClass("addon")) {
					jQuery(aEl[0]).removeClass("addon");
				}
				aEl[0].onfocus = function(e) {
					if(jQuery(this).parent().attr('id')) {
						jQuery(nav).find('li.active').each(function(e) {
							jQuery(this).removeClass('active');
						});
					}
				};
			...
			aEl[0].onblur = function(e) {
				if("모바일" == e.target.innerText) {
					jQuery(nav).hide();
					jQuery(nav).removeClass("active");
					jQuery(nav).prev().removeClass("selected");
					document.body.click();
				}
			}
		}
	...
	for(i = 0 ; i < topEl2.length ; i++){
		if(topEl2[i].id.substring(0,13) == "top-menu-head" ) {
			...
			if ( topEl2[i].id.substring(13) == depth1 ) {
				topEl2[i].className= topEl2[i].className + " active";
