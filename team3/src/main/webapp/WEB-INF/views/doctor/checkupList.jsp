					...
					<div class="mail-box">
						<aside class="lg-side">
							<div class="inbox-head">
								<h3> 검사의뢰 목록<small style="font-family: 'Do Hyeon', sans-serif; font-size: 25px;"> > 검사 의뢰서 > 검진서 등록</small>
								</h3>
											...
											<c:if test="${cnt>0 }">
												...
												<c:forEach var="dto" items="${dtos}">
													<c:forEach var="vo" items="${dto.checkup_kind}">
														<tr>
															<td>${dto.checkuplist}<c:set var="number" value="${number-1 }"></c:set>
															</td>
															<td><c:if test="${vo==1}">
																	<span class="label label-success">계층검사</span>
																</c:if> 
																<c:if test="${vo==2}">
																	<span class="label label-warning">요 검 사</span>
																</c:if> 
																<c:if test="${vo==3}">
																	<span class="label label-danger">혈액검사</span>
																</c:if> 
																<c:if test="${vo==4}">
																	<span class="label label-primary">영상검사</span>
																</c:if>
															</td>
															<td><a onclick="window.location='checkupInfo?pageNum=${pageNum}&number=${number+1}&checkuplist=${dto.checkuplist}&kind=${vo}'"
																	style="font-size: 20px;">${guestList[b].guestname} 님 </a>
															</td>
															<td>${docList[a].doctorname}</td>
															<td>
																<fmt:formatDate type="both" pattern="yyyy-MM-dd"
																	value="${dto.checkup_date}" />
															</td>
														</tr>
													</c:forEach>
