	   ...
	   <script>
			...
			function delLastTr( idN ){
				...
				if( tableObj.length!=1) {
					alert("id='"+idN + "' 을 가진 table 태그가 없어 delLastTr(~)란 이름의 함수 호출불가!"); return;
					}
					// 카피할 마지막 tr 태그를 관리하는 jQuery 객체 생성해 저장
				var lastTrObj = tableObj.find("tbody:eq(0) tr").first().parent().children().last();
		...
		//if(lastTrObj.parent().children("tr").length==1){
			...
			function addTr( idN ){
				...
				if( tableObj.length!=1)	{
					alert("id'="+idN + "'을 가진 table 태그가 없어 addTr(~)란 이름의 함수 호출 불가!");
					return;
				}
				// 카피할 마지막 tr 태그를 관리하는 jQuery 객체 생성해 저장
				var lastTrObj = tableObj.find("tbody:eq(0)").find("tr:eq(0)").parent().children().last();
					//checkbox와 radio 입력 양식을 가르키는 석택자 문자열 저장
					//------------------------------------
					var htmlType1 = "input[type=checkbox],input[type=radio]";
					// checkbox와 <APPLET CODE="" WIDTH="" HEIGHT="">
					// radio 입력 양식을 제외한 입력 양식 태그를 가르키는 선택자 문자열 저장
					var htmlType2 = 
					"select, textarea,input[type=text],input[type=password],input[type=hidden],input[type=image]";
				...
				var htmlType3 = htmlType1+ "," + htmlType2;
					...
					// 만약에 dataCnt 변수가 0이면
					if(dataCnt==0) {alert("마지막 행에 데이터가 비어있어 행 추가 불가능"); return; }
					//-------------------------------------------	
	            ...
	            <div class="well">
					<div>
						<div class="panel panel-primary">
						    ...
						    <table class="table table-striped table-hover" border="1" id="xxx">
						        <thead>
						            <tr>
						        ...
						        <tbody>
						            <tr>
						                ...
						                <td><input type="text" style="width:100%" min="0" max="99" name="drugrepeat1">번</td>
						                <td><input type="text" style="width:100%" min="0" max="99" name="dosagedate1">일</td>
									...
									<input type="button" value="행삭제" onclick="delLastTr( 'xxx' );">
						          	...
						          	<tr>
						                ...
						                <td><input type="text" style="width:100%" min="0" max="99" name="injectionrepeat1">번</td>
						                <td><input type="text" style="width:100%" min="0" max="99" name="injectiondate1">일</td>
						          	</tr>
						          	<!-- <tr>
							...
							<input type="button" value="행삭제" onclick="delLastTr( 'yyy' );">
