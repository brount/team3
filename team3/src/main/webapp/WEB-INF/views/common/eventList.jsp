								...
								<th align="center">
									<c:if test="${cnt>0}">
										...
										<c:if test="${startPage > pageBlock}">
											<a href="eventList" style="font-size: 15px;">[◀◀]</a>
											<a href="eventList?pageNum=${startPage-pageBlock}" style="font-size: 15px;">[◀]</a>	
										</c:if>
										<!-- 페이지 블록 -->
										<c:forEach var="i" begin="${startPage}" end="${endPage}">
											...
											<c:if test="${i != currentPage }">
												<a href="eventList?pageNum=${i}" style="font-size: 15px;">[${i}]</a>
											</c:if>
										</c:forEach>										
										<!-- 다음블록[▶] / 끝[▶▶] -->
										<c:if test="${pageCnt > endPage}">
											<a href="eventList?pageNum=${startPage+pageBlock}" style="font-size: 15px;">[▶]</a>
											<a href="eventList?pageNum=${pageCnt}" style="font-size: 15px;">[▶▶]</a>
										</c:if>
									</c:if>
								</th>
							</tr>
