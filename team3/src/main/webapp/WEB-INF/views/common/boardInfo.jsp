							...
							<ul class="responsive-table">
								<li class="li_q table-header">
									<div class="coll coll-1">
										<img src="resources/images/mainimg/q.png"
											style="width: 80px; height: 80px;">&nbsp;&nbsp;&nbsp;<b>${dto.boardtitle }</b>
									</div>
								</li>
								
								<li class="li_q table-row" style="display: flex;">
									<div class="coll coll-1" data-label="Job Id">작성자 :
										${dto.boardwriter }</div>
									<div class="coll coll-2" data-label="Customer Name">
										작성일 : <fmt:formatDate type="both" pattern="yyyy-MM-dd" value="${dto.boarddate}" />
									</div>
								</li>
							...
							<ul class="responsive-table">
								...
								<c:forEach var="dt" items="${dtos}">
									<li class="li_d table-row">
										...
										<div class="coll coll-3" data-label="Amount">
											...
											<c:if test="${sessionScope.id==dt.boardwriter}">
												<c:if test="${sessionScope.memberState==2}">
													<button class="btn btn-dark-blue"
														onclick="window.open('refDelete?num=${dt.boardno}&kind=${dto.boardno}&pageNum=${pageNum}&number=${number}','로그인창','width=450px,height=200px,screenX=700px,screenY=400px');">
														삭제
													</button>
												</c:if>
											</c:if>
								...
								<form action="inputre" method="post" name="reform" onsubmit="return reChk();">
									...
									<ul class="responsive-table">
										<li class="li_d table-row">
											...
											<div>
												<br>
												<input class="btn btn-dark-blue" type="submit" value="댓글작성">
											</div>
