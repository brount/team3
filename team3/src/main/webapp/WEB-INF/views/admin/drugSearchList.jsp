												...
												<th align="center">
													<c:if test="${cnt > 0}">
														...
														<c:if test="${startPage > pageBlock}">
															<a href="drugSearchList?sc=${sc}&search=${search}">[맨앞]</a>
															<a href="drugSearchList?pageNum=${startPage - pageBlock}&sc=${sc}&search=${search}">[이전]</a>
														</c:if>
														...
														<c:forEach var="i" begin="${startPage}" end="${endPage}">
															...
															<c:if test="${i != currentPage}">
																<a href="drugSearchList?pageNum=${i}&sc=${sc}&search=${search}">[${i}]</a>
															</c:if>
														</c:forEach>

														<!-- 맨끝[▶▶] / 다음▶] -->
														<c:if test="${pageCount > endPage}">
															<a href="drugSearchList?pageNum=${startPage + pageBlock}&sc=${sc}&search=${search}">[다음]</a>
															<a href="drugSearchList?pageNum=${pageCount}&sc=${sc}&search=${search}">[맨뒤]</a>
														</c:if>
													</c:if>
												</th>
											</tr>
