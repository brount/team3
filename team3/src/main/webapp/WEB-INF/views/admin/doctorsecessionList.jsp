...
<body>
	<div id="wrapper">
		...
		<div id="page-wrapper">
			<div class="row">
				<div class="col-lg-12">
					<h1 class="page-header">회원관리 - 의사탈퇴회원목록</h1>
					<!-- 페이지 제목 -->
				</div>
				<!-- /.col-lg-12 -->
			</div>
							...
							<div class="pull-right">
								<div class="btn-group">
									...
									<button type="button" class="btn btn-dark-blue"
										style="width: 50px; height: 30px; margin-left: 15px;"
										onclick="doctordeleteCheck();">삭제</button>
								</div>
							</div>
						</div>
						<!-- /.panel-heading -->
											...
											<c:if test="${cnt > 0}">
												<c:if test="${sc == null}">
													...
													<c:if test="${startPage > pageBlock}">
														<a href="doctorsecessionList"><<</a>
														<a href="doctorsecessionList?pageNum=${startPage - pageBlock}"><</a>
													</c:if>
													...
													<c:if test="${pageCount > endPage}">
														<a href="doctorsecessionList?pageNum=${startPage + pageBlock}">></a>
														<a href="doctorsecessionList?pageNum=${pageCount}">>></a>
													</c:if>
												</c:if>

												<c:if test="${sc != null}">
													<c:if test="${startPage > pageBlock}">
														<a href="doctorsecessionSearchList?sc=${sc}&search=${search}"><<</a>
														<a href="doctorsecessionSearchList?pageNum=${startPage - pageBlock}&sc=${sc}&search=${search}"><</a>
													</c:if>
													...
													<c:if test="${pageCount > endPage}">
														<a href="doctorsecessionSearchList?pageNum=${startPage + pageBlock}&sc=${sc}&search=${search}">></a>
														<a href="doctorsecessionSearchList?pageNum=${pageCount}sc=${sc}&search=${search}">>></a>
													</c:if>
												</c:if>
											</c:if>
										</div>

										<form action="doctorsecessionSearchList" class="search_box"
