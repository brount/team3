	...
	<div id="wrapper">
	   ...
	   <jsp:include page="./admin_nav.jsp"></jsp:include>
	    ...
	    <div id="page-wrapper">
	        <div class="row">
	            <div class="col-lg-12">	
	                <h1 class="page-header">회원관리 - 의사제제회원목록 </h1>		<!-- 페이지 제목 -->
	            </div>
	            <!-- /.col-lg-12 -->
	        </div>
	        <!-- /.row -->
	                        ...
	                        <div class="row">
	                            <div class="col-lg-4">
	                                <div class="table-responsive">
													...
													<c:if test="${cnt > 0}">
														<c:if test="${sc == null}">
															...
															<c:if test="${startPage > pageBlock}">
																<a href="doctorsanctionList">[맨앞]</a>
																<a href="doctorsanctionList?pageNum=${startPage - pageBlock}">[이전]</a>
															</c:if>
															...
															<c:forEach var="i" begin="${startPage}" end="${endPage}">
																...
																<c:if test="${i != currentPage}">
																	<a href="doctorsanctionList?pageNum=${i}">[${i}]</a>
																</c:if>
															</c:forEach>
															
															<!-- 맨끝[▶▶] / 다음▶] -->
															<c:if test="${pageCount > endPage}">
																<a href="doctorsanctionList?pageNum=${startPage + pageBlock}">[다음]</a>
																<a href="doctorsanctionList?pageNum=${pageCount}">[맨뒤]</a>
															</c:if>
														</c:if>
														
														<c:if test="${sc != null}">
															<c:if test="${startPage > pageBlock}">
																<a href="doctorsanctionSearchList?sc=${sc}&search=${search}">[맨앞]</a>
																<a href="doctorsanctionSearchList?pageNum=${startPage - pageBlock}&sc=${sc}&search=${search}">[이전]</a>
															</c:if>
															...
															<c:forEach var="i" begin="${startPage}" end="${endPage}">
																...
																<c:if test="${i != currentPage}">
																	<a href="doctorsanctionSearchList?pageNum=${i}&sc=${sc}&search=${search}">[${i}]</a>
																</c:if>
															</c:forEach>
															
															<c:if test="${pageCount > endPage}">
																<a href="doctorsanctionSearchList?pageNum=${startPage + pageBlock}sc=${sc}&search=${search}">[다음]</a>
																<a href="doctorsanctionSearchList?pageNum=${pageCount}sc=${sc}&search=${search}">[맨뒤]</a>
															</c:if>
														</c:if>
													</c:if>
												</th>
	                                    ...
	                                    <table align="center">
	                                    	<form action="doctorsanctionSearchList" class="search_box" method="get">
                                  			<tr>
                                       			<td>
