...
<body>
	<div id="wrapper">
		...
		<div id="page-wrapper">
			<div class="row">
				<div class="col-lg-12">
					<h1 class="page-header">회원관리 - 일반회원목록</h1>
					<!-- 페이지 제목 -->
				</div>
				<!-- /.col-lg-12 -->
			</div>
											...
											<c:if test="${cnt > 0}">
												<c:if test="${sc == null}">
													...
													<c:if test="${startPage > pageBlock}">
														<a href="memberList"><<</a>
														<a href="memberList?pageNum=${startPage - pageBlock}"><</a>
													</c:if>
													...
													<c:if test="${pageCount > endPage}">
														<a href="memberList?pageNum=${startPage + pageBlock}">></a>
														<a href="memberList?pageNum=${pageCount}">>></a>
													</c:if>
												</c:if>

												<c:if test="${sc != null}">
													<c:if test="${startPage > pageBlock}">
														<a href="memberSearchList?sc=${sc}&search=${search}"><<</a>
														<a href="memberSearchList?pageNum=${startPage - pageBlock}&sc=${sc}&search=${search}"><</a>
													</c:if>
													...
													<c:forEach var="i" begin="${startPage}" end="${endPage}">
														...
														<c:if test="${i != currentPage}">
															<a href="memberSearchList?pageNum=${i}&sc=${sc}&search=${search}">${i}</a>
														</c:if>
													</c:forEach>

													<c:if test="${pageCount > endPage}">
														<a href="memberSearchList?pageNum=${startPage + pageBlock}sc=${sc}&search=${search}">></a>
														<a href="memberSearchList?pageNum=${pageCount}sc=${sc}&search=${search}">>></a>
													</c:if>
												</c:if>
											</c:if>
										</div>

										<form action="memberSearchList" class="search_box"
											method="post" name="searchForm" onsubmit="searchChk()">
											<table align="center">
												<tr>
													<td>
